...
- `tools/read_page/html-reader.js` contains an inlined copy for CSP-safe private ISOLATED-world injection.
...
4. Copy the same implementation into `tools/read_page/html-reader.js` between the vendored-library markers, preserving the outer `/* eslint-disable */` guard and omitting the upstream CommonJS `module.exports` block so Rollup emits a classic script with no module export.
...
The synchronization test treats the two `_safeHTML(...)` wrappers and removal of the unused CommonJS export as the only permitted implementation differences between the canonical and inlined copies. Review any additional difference rather than normalizing it away: the wrappers are required by extension execution on Trusted Types-enforcing sites such as Gmail.
...
2. The private HTML reader host must be self-contained so `chrome.scripting.executeScript({ files: [...] })` can inject it into the exact document's ISOLATED world without violating the page's Content Security Policy.
